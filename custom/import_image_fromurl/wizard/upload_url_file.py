...
class UploadUrlFile(models.TransientModel):
    ...
    def upload_images_supplier_csv(self, csv_excel_file, map_field):
        ...
        line_list = base64.decodestring(csv_excel_file).split('\n')
        record_list = [line for line in line_list if line]
        if record_list:
            # Skip Header Line
            record_list.pop(0)
    ...
    def upload_images_supplier_xls(self, csv_excel_file, map_field):
        ...
        for row_number in range(sheet.nrows):
            if row_number <= 0:
                ...
                line = (
                    map(lambda row: isinstance(row.value, unicode) and row.value.encode('utf-8') or str(row.value), sheet.row(row_number)))
                if map_field == 'partner_id':
